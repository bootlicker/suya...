...
(defun eliza ()
  (loop
   ...
   (let* ((line (read-line))
	  ...
	  (setq *bindings* nil)
	  (setq *is-repeat* (append *is-repeat* (list (return-response input))))
	  (output-formatted-response (nth 0 (reverse *is-repeat*))))))

(defun output-formatted-response (input)
  (format t "~{~(~A ~)~}~%" input))

(defun return-response (input)
  (dolist (r *rules*)
